...
final class CarouselViewModel {
  ...
  func load() async {
    ...
    do {
      let newItems: [CarouselViewModel.Item]
      switch list {
      case .movies(let movieList):
        newItems = try await items(movieList: movieList)
      case .tvShows(let tvShowList):
        newItems = try await items(tvShowList: tvShowList)
      }
